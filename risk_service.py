...
"""
    ...
    6. 不得将用户未提供的企业事实自行补充为既有事实。合理推测必须使用条件性表述。
    7. 对"已识别风险"给出 2-3 项最重要、最直接的控制措施。
    8. 对"待核验风险"不要直接提出一整套治理体系，而是告诉用户需要核验什么，例如："核验不同群体误识率是否存在显著差异。"
    9. 对"当前控制较好的风险"明确肯定现有控制："当前已有本地处理和短期删除机制，建议持续监测，不需要新增复杂措施。"
    10. 当用户选择"其他通用领域"时，不得调用 ERS 分数、排名、Top 5 覆盖率或稳健性分层。
    """
